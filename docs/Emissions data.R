...
state_emissions <- ggplot(USA_Emissions_State, aes(fct_reorder(State, `Total reported direct emissions`),
                                                   ...
                                                   text = paste0("<b>State:", State, "<br>",
                                                                 "<b> Total Repoted direct emissions:",sprintf("%0.2f", round( `Total reported direct emissions`, digits = 2)),"<br>",
  ...
  geom_point()+
  coord_flip()+
  labs(y = "GHG Emissios in CO2e", x = "State", title = "Total Reported Direct Emissions")
...
Carbon_map <- tm_shape(USA_Emissions_State, unit = "km")+
  tm_polygons(col = "CO2 emissions (non-biogenic)",
              ...
              style = "sd",
              contrast=.3000,
               title="Metric Tonnes of Carbon Dioxide (CO2e)")
Carbon_map
...
Methane_map <- tm_shape(USA_Emissions_State, unit = "km")+
  tm_polygons(col = "Methane (CH4) emissions",
              ...
              style = "sd",
              contrast=.3000,
              title="Metric Tonnes of Methane (CO2e)")
...
Nitrous_map <- tm_shape(USA_Emissions_State, unit = "km")+
  tm_polygons(col = "Nitrous Oxide (N2O) emissions",
              ...
              style = "sd",
              contrast=.3000,
              title="Metric Tonnes of Nitrous Oxide (CO2e)")
Nitrous_map
...
sum(ghg2019$`Total reported direct emissions`, na.rm = TRUE) #2,622,343,176

#Post questions
##How much GHG emissions come from the USA's industrial sector?

##How much does this data set cover?

sum(ghg2019$`Total reported direct emissions`, na.rm = TRUE) #= 2,622,343,176 MTCO2e
sum(ghg2019$`Total reported direct emissions`, na.rm = TRUE)/(6558*(10^6)) #= 40% of reported emissions 


##How much US industrial emissions are produced by state?
###Dashboard
#### Findings
